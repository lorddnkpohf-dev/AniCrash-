...
async function startServer() {
      ...
      if (shikimoriRes.status === 'fulfilled' && Array.isArray(shikimoriRes.value)) {
        shikimoriRes.value.forEach((item: any) => {
          results.push({
            ...
            poster: item.image?.original ? `https://shikimori.one${item.image.original}` : 'https://shikimori.one/assets/globals/missing_original.png',
            year: parseInt(item.aired_on) || 0,
            score: parseFloat(item.score) || 8.0,
            source: 'shikimori',
            episodesCount: item.episodes || item.episodes_aired || 12,
            currentEpisodes: item.episodes_aired || item.episodes || 12,
            type: item.kind === 'movie' ? 'Фильм' : 'TV Сериал',
            status: item.status === 'released' ? 'Завершён' : 'Онгоинг',
          });
        });
      }
  ...
  // Custom Anime Database for anime not on Anilibria & Dynamic API Generation (JSON fallback)
  const getCustomDbPath = (accountId: string) => path.join(process.cwd(), `custom_anime_db_${accountId.replace(/[^a-z0-9]/gi, '_')}.json`);
  const DEFAULT_ACCOUNT = 'default';

  function loadCustomDb(accountId: string = DEFAULT_ACCOUNT): any[] {
    const dbFile = getCustomDbPath(accountId);
    if (!fs.existsSync(dbFile)) {
      try {
        fs.writeFileSync(dbFile, JSON.stringify(ANIME_DATABASE), 'utf-8');
        console.log(`Initialized ${dbFile} with ${ANIME_DATABASE.length} static anime entries.`);
  ...
  function saveCustomDb(accountId: string = DEFAULT_ACCOUNT, db: any[]) {
    try {
      fs.writeFileSync(getCustomDbPath(accountId), JSON.stringify(db), 'utf-8');
    } catch (err) {
  ...
  app.get('/api/custom-anime', async (req, res) => {
    ...
    const db = loadCustomDb('gagastudenikini40_gmail_com');
  ...
  app.post('/api/custom-anime', async (req, res) => {
    try {
      ...
      // JSON file fallback
      let db = loadCustomDb('gagastudenikini40_gmail_com');
      ...
      saveCustomDb('gagastudenikini40_gmail_com', db);
