          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 text-center sm:text-left">
              ...
              <div className="relative shrink-0">
                <div
                  className={`w-28 h-28 sm:w-32 sm:h-32 rounded-full overflow-hidden border-3 bg-zinc-900 transition-all duration-300 ${activeAura.className}`}
                >
                  <EnhancedImage
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {AURA_PRESETS.map((aura) => {
                ...
                const isLocked = (aura.episodesRequired || 0) > totalEpisodesWatched;
                return (
                  <button
                    key={aura.id}
                    disabled={isLocked}
                    onClick={() => !isLocked && setDraftProfile((p) => ({ ...p, aura: aura.id }))}
                    ...
                    {isLocked && (
                      <div className="absolute inset-0 flex items-center justify-center bg-black/60 z-10 backdrop-blur-sm">
                        <div className="flex flex-col items-center gap-1 text-white">
                          ...
                          <span className="text-[10px] font-bold">{aura.episodesRequired} сер.</span>
                        </div>
                      </div>
                    )}
                    <div className={`flex items-center gap-3 ${isLocked ? 'grayscale' : ''}`}>
                      <div
