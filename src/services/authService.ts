...
export async function registerAccount(
  ...
  // 1. SignUp locally (since Supabase is disabled)
...
export async function loginAccount(email: string, password: string): Promise<AuthResponse> {
  ...
  // 1. SignIn locally (since Supabase is disabled)
  const users = getRegisteredUsers();
  ...
  // NOTE: Simple password simulation for local-only auth
  // In a real local-only app you'd need hashed password storage
  setSession(user);
