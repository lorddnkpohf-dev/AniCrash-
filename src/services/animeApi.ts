...
export async function fetchFullShikimoriDetails(shikimoriId: string): Promise<Partial<Anime> | null> {
  try {
    const res = await fetch(`https://shikimori.one/api/animes/${shikimoriId}`);
    if (res.ok) {
      const sData = await res.json();
      return {
        description: sData.description || 'Описание отсутствует.',
        genres: (sData.genres || []).map((g: any) => g.russian || g.name),
        banner: sData.image?.original ? `https://shikimori.one${sData.image.original}` : '',
        studio: (sData.studios || []).map((s: any) => s.name).join(', ') || 'Неизвестно',
        ageRating: sData.rating || '16+',
        originalTitle: sData.name || '',
        season: sData.season || 'Неизвестно',
      };
    }
  } catch (err) {
    console.error('Failed to fetch full Shikimori details:', err);
  }
  return null;
}
