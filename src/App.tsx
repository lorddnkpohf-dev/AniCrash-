...
import { ensureAnimeEpisodes, fetchFullShikimoriDetails } from './services/animeApi';
...
export default function App() {
  ...
  const [shikimoriSearchResults, setShikimoriSearchResults] = useState<Anime[]>([]);
  ...
  // Online search from Shikimori
  useEffect(() => {
    if (!debouncedSearchQuery.trim()) {
      setShikimoriSearchResults([]);
      return;
    }

    const searchShikimori = async () => {
      setIsSearchLoading(true);
      try {
        const res = await fetch(`/api/search?query=${encodeURIComponent(debouncedSearchQuery.trim())}`);
        if (res.ok) {
          const data = await res.json();
          const mapped: Anime[] = data.map((item: any) => ({
            id: item.id,
            title: item.title,
            englishTitle: item.englishTitle || item.title,
            originalTitle: item.englishTitle || item.title,
            slug: item.id,
            description: `Источник: ${item.source || 'shikimori'}`,
            ...
            year: item.year || 2024,
            season: 'Неизвестно',
            type: (item.type || 'TV Сериал') as any,
            status: (item.status || 'Завершён') as any,
            genres: ['Аниме'],
            episodesCount: item.episodesCount || 12,
            currentEpisodes: item.currentEpisodes || 12,
            durationPerEp: '24 мин.',
            studio: 'Студия',
            ageRating: '16+',
            voiceovers: ['Kodik', 'AniLibria'],
            episodes: [],
            characters: [],
            tags: [item.source || 'shikimori'],
            trendingRank: 0
          }));
          setShikimoriSearchResults(mapped);
        }
      } catch (err) {
        console.error('Failed to search Shikimori:', err);
      } finally {
  ...
  const [selectedSource, setSelectedSource] = useState<'all' | 'anilibria' | 'kodik' | 'other'>('all');
  ...
  const filteredCatalog = useMemo(() => {
    return completeAnimeDatabase.filter((anime) => {
      // Source filter
      if (selectedSource === 'anilibria' && !anime.tags.includes('AniLibria')) return false;
      if (selectedSource === 'kodik' && !anime.tags.includes('Kodik')) return false;
      if (selectedSource === 'other' && (anime.tags.includes('AniLibria') || anime.tags.includes('Kodik'))) return false;
  ...
  }, [debouncedSearchQuery, selectedGenre, selectedStatus, selectedType, selectedSort, selectedSource]);

  const catalogList = useMemo(() => {
    if (debouncedSearchQuery.trim()) {
      return shikimoriSearchResults;
    }
    return filteredCatalog;
  }, [debouncedSearchQuery, shikimoriSearchResults, filteredCatalog]);
  ...
  const handleOpenAnimeDetail = useCallback(async (anime: Anime) => {
    setSelectedAnimeModal(anime);
    // If it's a Shikimori anime and has no full description/genres, fetch details
    if (anime.id && /^\d+$/.test(anime.id) && (!anime.description || anime.description.startsWith('Источник:'))) {
      try {
        const details = await fetchFullShikimoriDetails(anime.id);
        if (details) {
          setSelectedAnimeModal((prev) => {
          ...
          <div className="space-y-6">
            ...
            {isSearchLoading ? (
              <div className="flex flex-col items-center justify-center py-20 gap-3">
                ...
                <p className="text-zinc-400 text-sm font-medium">Глобальный поиск по базе Shikimori...</p>
              </div>
              ...
              <div className="space-y-8">
                <div className="grid grid-cols-2 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-3 sm:gap-4 lg:gap-6">
                  {catalogList.slice(0, catalogVisibleCount).map((anime) => (
                    <AnimeCard
                      key={anime.id}
                      anime={anime}
                      isFavorite={isFavorite(anime.id)}
                      progress={watchHistory.find((w) => w.animeId === anime.id)}
                      onSelect={(a) => handleOpenAnimeDetail(a)}
                      onPlay={(a) => handlePlayAnime(a)}
                      onToggleFavorite={handleToggleFavorite}
                    />
                  ))}
                ...
                {catalogVisibleCount < catalogList.length && (
